...
namespace Kiwi.Data.Collections
{
    ...
    public class Catalog<TKey, TEntry> : Catalog, IEnumerable<TEntry>
    {
        ...
        CollectionReferenceGetter source;
        KeyGetter keyFrom;
        ...
        int GoodDictionarySize (int candidate)
        {
        ...
        static (int[] buckets, BucketEntry[] bucketEntries) CreateBucketsAndEntries (int capacity)
        {
        ...
        static void FillBuckets (int[] oldBuckets, BucketEntry[] oldBucketEntries, int[] newBuckets, BucketEntry[] newBucketEntries)
        {
        ...
        void FillBuckets (TEntry[] source, int[] newBuckets, BucketEntry[] newBucketEntries)
        {
        ...
        int Grow () => EnsureCapacity(count * 2);

        int TargetBucket(int hashCode) => (hashCode & 0x7FFFFFFF) /* no negative values */ % buckets.Length;
        static int TargetBucket(int hashCode, int numBuckets) => (hashCode & 0x7FFFFFFF) /* no negative values */ % numBuckets;

        void Insert(TKey key, TEntry value, bool throwOnOverwrite)
        {
            ...
            { // No free entries, means that 'count' will also point to the
        ...
        internal Catalog(CollectionReferenceGetter source, KeyGetter keyGetter) : this(source, keyGetter, EqualityComparerForUnity<TKey>.Default)
        {
            
        }

        internal Catalog(CollectionReferenceGetter source, KeyGetter keyGetter, IEqualityComparer<TKey> comparer) {
            this.Keys = new KeyCollection(this);
        ...
        static class CatalogEnumerator {
